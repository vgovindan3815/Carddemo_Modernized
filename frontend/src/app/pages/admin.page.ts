...
@Component({
  ...
  template: `
    <h2>Admin Menu</h2>
    <p class="intro">Choose an administrative operation.</p>
    <div class="grid">
      <a class="tile" *ngFor="let option of options" [routerLink]="resolveRoute(option.route)">{{ option.option }}. {{ option.label }}</a>
    </div>
  `,
  styles: [
...
export class AdminPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getAdminMenu().subscribe((res) => this.options = res.options || []);
  }

  resolveRoute(route: string): string {
    return route.includes(':userId') ? route.replace(':userId', 'U0000001') : route;
  }
}
